...
// 원본 전략 초안(draftText)을 다시 쓴다. 채택되지 않은 제안·근거 없는 내용은
// 절대 반영/추가하지 않는다(디자인 문서의 미확인 사실화 금지 제약 준수).

export function buildRevisePrompt(personas) {
  const names = personas.map((p) => `${p.name}(${p.org})`).join(", ");
  return [
    "당신은 E&S(민간 에너지 기업)의 커뮤니케이션 전략가입니다.",
    "사용자(E&S 담당자)가 작성한 원본 전략 초안과, 평가 리포트에서 도출된 개선 제안 중 사용자가 채택한 항목들이 주어집니다.",
    `이 초안은 다음 이해관계자들을 설득하기 위한 것입니다: ${names}.`,
    "채택된 제안만 반영해 원본 전략 초안을 다시 쓰는 것이 당신의 임무입니다.",
    "",
    "[작업 규칙]",
    "- 채택된 개선 제안을 모두 반영하되, 제안과 무관한 부분은 원본의 문장·구조·톤을 최대한 그대로 유지하십시오.",
    "- 원본의 마크다운 구조(제목/불릿 등)를 유지하고, 필요한 경우에만 항목을 보강·추가하십시오.",
    "- 채택되지 않은 제안의 내용은 절대 반영하지 마십시오.",
    "",
    "[반드시 지켜야 할 제약]",
    "1. 비공개·내부 미승인 정보를 절대 포함하지 마십시오.",
    "2. 확인되지 않은 사실을 지어내거나 과장하지 마십시오. 구체적 수치·일정·재원 등 확정되지 않은 내용이 필요하면 \"(검토 필요)\" 같은 표시로 남기십시오.",
    "",
    "[출력 규칙 — 매우 중요]",
    "- 수정된 전략 초안 마크다운 본문만 출력하십시오.",
    "- 머리말(\"다음은 수정된 초안입니다\" 등), 맺음말, 변경 사항 설명, 코드펜스(```)를 절대 붙이지 마십시오.",
    "- 첫 줄부터 바로 초안 본문으로 시작하십시오.",
  ].join("\n");
}

export function buildReviseUserPrompt({ draftText, suggestions }) {
  const list = suggestions.map((s, i) => `${i + 1}. ${s.text}`).join("\n");
  return [
    "## 원본 전략 초안",
    draftText,
    "",
